...
- **Git管理可能なYAML**：プロジェクトは `.tally/` ディレクトリに保存、バージョン管理できる
...
**Phase 4 完了** (2026-04-19 時点): AI Engine 基盤実装済み。UC ノードから「ストーリー分解」アクションで Claude が userstory proposal を生成し、人間が採用することで正規 userstory に昇格するワークフローが動作する。
...
1. 対象リポジトリに `.tally/` を配置 (または `examples/sample-project` を使う)
2. ブラウザで http://localhost:3000 を開き、プロジェクトを選択
3. UC ノードを選択 → DetailSheet の「ストーリー分解」ボタンを押下
4. AgentProgressPanel に進捗が流れ、破線の proposal ノードが 1〜7 個生成される
5. 各 proposal を選択 → 「採用する」で userstory に昇格
