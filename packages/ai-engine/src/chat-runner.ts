...
// 1 user turn の間だけ生きる mutable state。
// long-lived Query から流れてくる SDK メッセージを「今どの assistant message に紐付けるか」
...
interface TurnState {
  ...
  // 外部 MCP の id → name の即引き map (label 表示用、turn ごとに固定で再構築しない)。
...
export class ChatRunner {
  ...
  // 再起動するまで mcpServers の入替えは反映しない (実 SDK は setMcpServers で動的更新できるが
  // MVP では undo/redo を許さず、変更は次回 ChatRunner 起動から有効にする)。
  ...
  async *runUserTurn(userText: string, contextNodeIds: string[] = []): AsyncGenerator<ChatEvent> {
    ...
    // 4. turn state を組み立てる。出力ループ (runOutputLoop) と MCP ハンドラはここから
    //    assistantMsgId / queue を読む。turn 中は this.currentTurn = 同じインスタンス。
    ...
    // 5. SDK Query (long-lived) を必要なら起動する。OAuth state を turn 跨ぎで保持するため
    //    1 ChatRunner = 1 sdk.query()。失敗 (mcpServers 設定不正等) は error を yield して終わる。
    try {
    ...
    // 6. user message を SDK の input ストリームに push する。実 SDK は streaming input mode で
    //    待機しており、ここで turn が始まる。出力ループが SDK 応答を queue に流し込む。
    if (this.input) {
      this.input.push({
        type: 'user',
    ...
    //    MCP ハンドラから push される pending/result も同じ queue を通る。
    try {
      while (true) {
      ...
      this.currentTurn = null;
      // queue は finish しない: 出力ループから pending な MCP イベントが遅延で push される
      // 可能性は無いが、念のため明示的に終わらせる必要は無い (turn が抜けた後は誰も読まない)。
    }
  }
  ...
  private async ensureQuery(): Promise<void> {
    if (this.query && !this.outputLoopFailed) {
      console.log('[chat-runner] ensureQuery: reuse existing query');
      return;
    }
    console.log(
      '[chat-runner] ensureQuery: creating new query (failed?',
      this.outputLoopFailed,
      ')',
    );
  ...
  private async runOutputLoop(query: SdkQueryHandle): Promise<void> {
    console.log('[chat-runner] runOutputLoop: started');
    try {
      ...
      console.log('[chat-runner] runOutputLoop: error', err);
    ...
    console.log(
      '[chat-runner] runOutputLoop: iter ended (currentTurn?',
      this.currentTurn !== null,
      ')',
    );
    // iter 正常終端 (close 等)。進行中 turn が残っていれば打ち切る。
    // 同時に query が死んだ印として outputLoopFailed を立て、次回 ensureQuery で作り直させる。
    this.outputLoopFailed = true;
    const turn = this.currentTurn;
    if (turn) {
      turn.queue.push({ type: 'chat_turn_ended' });
  ...
  private async dispatchSdkMessage(msg: SdkMessageLike): Promise<void> {
    ...
    const { assistantMsgId, queue, textBuffer, stashedAuthUses, externalConfigById } = turn;
  ...
  async close(): Promise<void> {
    this.tearDownQuery();
    if (this.outputLoopDone) {
      // 念のため出力ループの終了を待つ (リソースリーク防止)。
      try {
        await this.outputLoopDone;
      } catch {
        /* swallow */
      }
    }
  ...
  private async handleAuthToolResult(opts: {
    ...
    if (match.kind === 'authenticate') {
      ...
      if (!authUrl) {
        // URL 抽出に失敗 (フォーマット変更 / 認証 server エラー等)。failed として可視化。
  ...
  private buildMcpServer(tools: ToolEntry[]) {
    ...
    // long-lived MCP server: handler 呼び出し時の「現 turn」から assistantMsgId / emit を読む。
    // SDK は tool 呼び出し中ずっと query を block するので、その間 currentTurn は不変が保証される。
    // turn が無い (= 想定外) ときは tool 呼び出しを失敗で返して保身する。
    const makeHandler = (name: string) => async (input: unknown) => {
      const entry = find(name);
      const turn = this.currentTurn;
      if (!turn) {
        return {
          content: [{ type: 'text' as const, text: 'no active turn (chat runner 状態異常)' }],
          isError: true,
        };
      }
